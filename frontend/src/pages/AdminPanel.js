...
export default function AdminPanel() {
  ...
  const resolveUserNames = useCallback(async (msgs) => {
    ...
    try {
      ...
      console.warn('Failed to resolve user names');
    }
  }, []);
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [statsRes, msgsRes] = await Promise.all([
        ...
        api.get('/admin/messages?page=1&limit=20').catch(() => {
          console.warn('Failed to load admin messages');
          return null;
        }),
      ]);
      ...
      console.warn('Failed to load admin dashboard');
    }
    setLoading(false);
  }, [resolveUserNames]);
