...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    client.on('NEW_MESSAGE', (event) => {
      ...
      setAllConversations(prev => {
        ...
        }).catch(() => console.warn('Failed to fetch user for new message'));
        return prev;
      });
    });
  ...
  useEffect(() => {
    ...
    api.get(`/users/${userId}`).then(({ data }) => setActiveChat(data)).catch(() => console.warn('Failed to load user from params'));
  ...
  useEffect(() => {
    if (activeChat) {
      api.get(`/messages/conversations/${activeChat.id}`)
        ...
        .catch(() => console.warn('Failed to load messages for active chat'));
    }
  }, [activeChat]);
