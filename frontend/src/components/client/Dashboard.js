...
export default function ClientDashboard() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setRecommendedFreelancers(recData.data?.freelancers || []);
    } catch {
      console.warn('Failed to load client dashboard data');
    }
  }, []);
  ...
  const handleAcceptProposal = async (proposalId) => {
    try {
      ...
      showToast(dispatch, err.response?.data?.detail || 'Failed to accept proposal', 'error');
    }
  };
  ...
  const handleRejectProposal = async (proposalId) => {
    try {
      ...
      showToast(dispatch, err.response?.data?.detail || 'Failed to reject proposal', 'error');
    }
  };
