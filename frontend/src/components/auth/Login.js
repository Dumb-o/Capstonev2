...
export default function Login() {
  ...
  const { connectAndCheck, walletLogin, walletRegister, registerEmail, loginEmail, loading, error } = useAuth();
  ...
  const handleMetaMask = async () => {
    ...
    try {
      const meta = await connectAndCheck();
      if (meta.exists) {
        await walletLogin(meta.address, meta.provider);
      } else {
        setWalletMeta(meta);
      }
    } catch {
  ...
  const handleWalletConfirm = async () => {
    ...
    await walletRegister(walletMeta.address, walletMeta.provider, role);
    setWalletMeta(null);
  };

  const walletRegisterUI = walletMeta && (
    <div className="wallet-register-flow" style={{ marginTop: 20 }}>
      <p style={{ fontSize: 13, color: 'var(--gray)', marginBottom: 16 }}>
        Wallet connected: {walletMeta.address.slice(0,6)}...{walletMeta.address.slice(-4)}
      </p>
      ...
      <button className="btn btn-primary btn-full" onClick={handleWalletConfirm} disabled={loading}>
        {loading ? 'Creating account...' : 'Create Account'}
      </button>
      <button className="btn btn-ghost btn-full" onClick={() => setWalletMeta(null)} disabled={loading} style={{ marginTop: 8 }}>
        Cancel
      </button>
    </div>
  );
        ...
        <div className="auth-form-wrap">
          ...
          {tab === 'signin' ? (
            <div className="form-panel active" style={isAdminMode ? { paddingTop: 40 } : {}}>
              ...
              {!walletMeta ? (
                <button className="btn btn-outline btn-full" onClick={handleMetaMask} disabled={loading || walletLoading}>
