...
async def create_contract(
    ...
    if data.freelancer_id:
        ...
        if freelancer:
            if not freelancer.wallet_address:
                raise ValidationError(
                    "Freelancer has no wallet address configured. "
                    "They must connect a wallet before an on-chain contract can be created.",
                    code=ErrorCodes.VALIDATION_NO_FREELANCER_WALLET,
                )
