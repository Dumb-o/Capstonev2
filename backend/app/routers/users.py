...
from app.schemas.schemas import PaginatedUsers, UserResponse, UserUpdate
from app.utils.error_codes import ErrorCodes
from app.utils.exceptions import NotFoundError
from app.utils.helpers import pagination_params

_VALID_EXPERIENCE_LEVELS = {"beginner", "intermediate", "advanced", "expert"}
...
async def list_users(
    role: str | None = Query("freelancer"),
    ...
    if experience_level and experience_level not in _VALID_EXPERIENCE_LEVELS:
        raise HTTPException(
            status_code=422,
            detail=f"Invalid experience_level. Must be one of: {', '.join(sorted(_VALID_EXPERIENCE_LEVELS))}",
        )
    ...
    if skills:
        ...
        if skill_list:
            conditions = [cast(User.skills, String).ilike(f"%{s}%") for s in skill_list]
    ...
    if industry:
        pattern = f"%{industry}%"
        query = query.where(cast(User.industries, String).ilike(pattern))
