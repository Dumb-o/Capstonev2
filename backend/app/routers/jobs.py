...
from app.schemas.schemas import (
    JobCreate, JobUpdate, JobResponse, PaginatedJobs,
)
...
async def update_job(
    ...
    if job.client_id != current_user.id:
        raise AuthorizationError("Only the job poster can update")
...
async def close_job(
    ...
    if not job:
        raise NotFoundError("Job not found")
    if job.client_id != current_user.id:
        raise AuthorizationError("Only the job poster can close")
