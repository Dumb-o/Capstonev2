...
from app.models.models import Contract, ContractMilestone, ContractStatus, Job, User
...
async def create_contract(
    ...
    if abs(total_milestone_amount - data.total_amount) > 0.001:
        raise ValidationError("Milestone amounts must sum to total", code=ErrorCodes.VALIDATION_MILESTONE_SUM)
...
async def sign_contract(
    ...
    if contract.client_signed and contract.freelancer_signed:
        await NotificationService.create(
            db, contract.client_id, "contract_signed",
            f"Contract signed: {contract.title}",
            f"Both parties have signed {contract.title}. Ready for funding!",
        )
        await NotificationService.create(
            db, contract.freelancer_id, "contract_signed",
            f"Contract signed: {contract.title}",
            f"Both parties have signed {contract.title}. Ready for funding!",
        )
    else:
        ...
        if other_id:
            await NotificationService.create(
                db, other_id, "contract_signed",
                f"{contract.title} has been signed",
                f"{current_user.username or 'The other party'} has signed {contract.title}.",
            )
...
async def fund_contract(
    ...
    if contract.freelancer_id:
        await NotificationService.create(
            db, contract.freelancer_id, "contract_funded",
            f"Contract funded: {contract.title}",
            f"{contract.title} has been funded with {contract.total_amount} ETH. Work can begin!",
        )
