...
async def create_proposal(
    ...
    _: None = Depends(RoleRequire("freelancer")),
):
...
async def update_proposal_status(
    ...
    freelancer_name = freelancer.username if freelancer else "A freelancer"
    job_title = job.title if job else "a job"

    if new_status == "accepted":
        await NotificationService.create(
            db, proposal.freelancer_id, "proposal_accepted",
            f"Proposal accepted for {job_title}",
            f"Your proposal for {job_title} ({proposal.bid_amount} ETH) has been accepted by {current_user.username or 'the client'}! A contract has been created.",
        )
    elif new_status == "rejected":
        await NotificationService.create(
            db, proposal.freelancer_id, "proposal_rejected",
            f"Proposal not selected for {job_title}",
            f"Your proposal for {job_title} was not selected. Keep applying to other opportunities!",
        )
