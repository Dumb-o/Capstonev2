...
from app.models.models import Job, Message, Thread, User
...
async def _get_or_create_thread(
    db: AsyncSession, user_id: str, other_id: str,
) -> Thread | None:
    from sqlalchemy import and_, or_
    result = await db.execute(
        select(Thread).where(
            or_(
                and_(Thread.client_id == user_id, Thread.freelancer_id == other_id),
                and_(Thread.client_id == other_id, Thread.freelancer_id == user_id),
            )
        ).limit(1)
    )
    ...
    if not thread:
        result = await db.execute(
            select(Job).where(
                or_(Job.client_id == user_id, Job.client_id == other_id)
            ).limit(1)
        )
        job = result.scalar_one_or_none()
        if job:
            client_id = job.client_id
            freelancer_id = other_id if job.client_id == user_id else user_id
            thread = Thread(
                client_id=client_id,
...
async def send_message(
    ...
    thread = await _get_or_create_thread(db, current_user.id, data.receiver_id)
