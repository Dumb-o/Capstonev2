...
async def delete_job(
    ...
    # Remove dependents first to satisfy foreign key constraints
    await db.execute(delete(Proposal).where(Proposal.job_id == job_id))
    contract_ids = (await db.execute(
        select(Contract.id).where(Contract.job_id == job_id)
    )).scalars().all()
    if contract_ids:
        await db.execute(
            delete(ContractMilestone).where(ContractMilestone.contract_id.in_(contract_ids))
        )
